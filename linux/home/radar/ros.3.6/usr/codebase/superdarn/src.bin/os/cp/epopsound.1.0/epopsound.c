/* epopsound.c
   ============
   Author: K. Sterne based on epopsound written for ROS.1.25 by K. Krieger
*/
...
/*      if (bmnum==ebm) break;
	...
	if (bmnum >= ebm) {
      ...
      /* Lets read the time and see if we have enough to do more scanning */
      TimeReadClock(&yr,&mo,&dy,&hr,&mt,&sc,&us);
      /* If we're approaching the end of the minute, lets give the radar a breather */
      if( sc > 55 ) break;
